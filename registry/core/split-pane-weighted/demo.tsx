        ...
        <SashWeight
          ...
          start={
            <div className="h-full overflow-y-auto bg-background px-3 py-3">
              <p className="mb-2 px-1 font-mono text-[10px] uppercase tracking-wider text-ns-muted">
                Explorer
              </p>
              <ul>
                {FILES.map((f) => (
                  <li
                    ...
                    className={`truncate rounded-sm px-1.5 py-1 font-mono text-xs ${
                      f.active ? "bg-border text-foreground" : f.dir ? "text-foreground" : "text-ns-muted"
                    }`}
              ...
              start={
                <div className="h-full overflow-y-auto bg-background px-4 py-3 font-mono text-xs leading-6 text-foreground">
                  {CODE_LINES.map((line, i) => (
                    <div key={i} className="flex gap-3">
                      <span className="w-5 shrink-0 text-right text-ns-muted">{i + 1}</span>
                ...
                <div className="flex h-full flex-col items-center justify-center gap-3 bg-background px-4 py-3">
                  ...
                  <div className="w-full max-w-[220px] rounded-md border border-border bg-surface p-4">
                    <div className="mb-2 h-2 w-2/3 rounded-full bg-ns-muted opacity-40" />
                    <div className="mb-1 h-2 w-full rounded-full bg-ns-muted opacity-20" />
                    <div className="h-2 w-4/5 rounded-full bg-ns-muted opacity-20" />
                    <div className="mt-3 h-7 w-20 rounded-sm bg-ns-accent" />
                  </div>
                </div>
              }
            />
