...
import { RollingShutterSkew } from "./component";

// The grid fills the whole viewport and is already mid-bend before anything
// is touched — the pan oscillates on its own, the pointer only nudges the
// same shared velocity term it already runs on.
export default function RollingShutterSkewDemo() {
  return (
        ...
        <div className="pointer-events-none flex h-full w-full flex-col items-center justify-center gap-4 px-6 text-center">
          <div className="flex flex-col items-center gap-3 rounded-lg bg-background/70 px-7 py-5 backdrop-blur-md">
            <p className="font-mono text-[11px] uppercase tracking-[0.28em] text-ns-muted">
              ns-ui / rolling-shutter-skew
            </p>
            <h1 className="text-2xl font-medium tracking-tight text-foreground sm:text-3xl">
              Every row, a moment later
            </h1>
            <p className="max-w-sm text-sm text-ns-muted">
              A row-by-row sensor readout bends a straight grid into a
              shifting parallelogram as the simulated pan sweeps back and
              forth.
            </p>
          </div>
          <a
