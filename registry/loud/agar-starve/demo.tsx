...
import { AgarStarve } from "./component";

// The plate grows on its own; everything here is ordinary DOM type on
// tokens, sitting over it behind a scrim so the copy stays legible over
// whatever tone the biomass has reached underneath.
export default function AgarStarveDemo() {
  return (
        ...
        <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-end gap-4 px-6 pb-14 text-center sm:pb-20">
          <div className="flex flex-col items-center gap-3 rounded-lg bg-background/80 px-7 py-5 backdrop-blur-md">
            ...
            <p className="max-w-sm text-sm text-foreground sm:text-base">
              A colony reads its own depleting nutrient field: rich plate,
              round growth — starved plate, branching fingers.
            </p>
          </div>
          <a
            href="#docs"
