...
import { MagnetronRacetrackSweep } from "./component";

// The target already carries a faint groove from before mount (a short
// warmup), so the field never opens on a suspiciously pristine disc. Type
// sits on a token scrim since the disc spans most of the value range.
export default function MagnetronRacetrackSweepDemo() {
  return (
    <main className="h-screen w-full overflow-hidden bg-background">
      <MagnetronRacetrackSweep>
        <div className="pointer-events-none flex h-full w-full flex-col items-center justify-end gap-4 px-6 pb-14 text-center sm:pb-20">
          <div className="flex flex-col items-center gap-3 rounded-lg bg-background/70 px-7 py-5 backdrop-blur-md">
            <p className="font-mono text-[11px] uppercase tracking-[0.28em] text-ns-muted">
              ns-ui / magnetron-racetrack-sweep
            </p>
            <h1 className="text-2xl font-medium tracking-tight text-foreground sm:text-3xl">
              Worn by the sweep
            </h1>
            <p className="max-w-sm text-sm text-ns-muted">
              The racetrack sweeps in and out over 22s, slowly grooving the target wherever it lingers.
            </p>
          </div>
          <a
            href="#docs"
