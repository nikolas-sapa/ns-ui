...
import { WeldPool } from "./component";

// The headline is rasterized into the shader's height field, so it is lit by
// the same environment as the pool. Everything else is ordinary DOM type on
// tokens, sitting under it.
export default function WeldPoolDemo() {
  return (
    <main className="h-screen w-full overflow-hidden bg-background">
      <WeldPool headline={"MOLTEN\nSTATE"} headlineY={0.4}>
        {/* a token scrim, not a colour literal — the pool underneath spans the
            full value range in both themes, so unbacked type would sit on
            near-white and near-black within the same line */}
        <div className="pointer-events-none absolute inset-x-0 bottom-0 flex flex-col items-center gap-4 px-6 pb-14 text-center sm:pb-20">
          <div className="flex flex-col items-center gap-3 rounded-lg bg-background/70 px-7 py-5 backdrop-blur-md">
            ...
            <p className="max-w-sm text-sm text-foreground sm:text-base">
              Drag a wake through the surface and watch the letters take the light.
            </p>
          </div>
          <a
            href="#docs"
