...
and `public/llms-full.txt` ARE gitignored too (see `.gitignore`) and get regenerated on every build,
locally and on Vercel, so there is nothing to keep in sync by hand.

## Autoplay — how a card demonstrates itself

Landing-page cards are iframes onto `/preview/<name>?embed=1&autoplay=1` (served in practice by
`/preview/<name>/embed`, its cacheable path-form equivalent), scaled down and
...
`meta.json`, and one shared driver (`app/_components/autoplay-driver.tsx`) synthesises that
...
**Where it runs:** only when *both* `embed=1` and `autoplay=1` are present. `/components/<name>` (and
`/preview/<name>`, its chrome-less verifier twin) with
...
| `type` | `text`: string to type (**required**)<br>`cps`: characters/sec (default 3; use 2 when the per-key motion is the point)<br>`hold`: ms the full string rests before it clears and loops (default 1600)<br>`target`: input selector (default `input, textarea, [contenteditable]`) | Types `text` into the target, one real key sequence per character — cancelable `keydown`, then (only if the component did **not** `preventDefault` it) `beforeinput` + native-setter value write + `input`, then `keyup`. That fork drives both keyboard-first controls that own `keydown` (otp-reel) and ordinary controlled inputs. When `target` matches **several** elements (a per-box OTP), character k goes to match k — the demo is `inert`, so the component's own focus-advance is a no-op and every key would otherwise pile into box 1. Wakes anything that only animates on keystroke. Note: a demo that self-completes on the typed value (otp-reel verifies on `481632` and disables) types in once and rests, replaying when the card remounts, rather than looping. |
...
`"target": "button"` over dock-cursor-magnify's nine items unions to exactly the dock row. Negative
...
**Point `expect` at the thing that must be seen, not at a wrapper.** dropdown-drape's `[role=menu]`
...
### After adding components: `npm run order:build`

`lib/component-order.json` is the recency rank behind the catalog's **Newest** sort. It is
committed and regenerated by hand — deliberately NOT chained into `registry:build`, because it is
derived from git history and a Vercel shallow clone cannot be trusted to produce it (see the header
of `scripts/build-order.ts`).

The consequence of forgetting is backwards and easy to miss: an unranked component sorts **last**
under Newest rather than first, so the component you just built is the hardest one in the catalog to
find. This has now happened twice — 16 components, then 22 more — so `build-registry.ts` prints a
loud WARNING naming every unranked slug. It is a warning and not a hard failure on purpose: a deploy
that legitimately ships an unranked component should still deploy.

```bash
npm run order:build          # then commit lib/component-order.json
```

### Demos must not write host-shared browser state

A demo runs inside a same-origin iframe on the catalog, and the autoplay driver drives it on a loop
as a visitor scrolls. `localStorage` is shared across same-origin frames, so the iframe boundary
contains DOM changes but **not** storage writes.

`toggle-theme-ascii` shipped with its default `storageKey` of `ns-ui-theme` — byte-identical to the
site's own `THEME_STORAGE_KEY` in `lib/theme.ts`. Every autoplay press overwrote the visitor's real
theme preference, so the site flipped light/dark seemingly at random while scrolling the catalog.
Fixed by overriding the key in the demo only; the component keeps its behaviour.

If a component writes to `localStorage`, namespace the key (`command-palette-rotary` does this
correctly: `ns-command-palette-rotary:<key>`) and make sure its **demo** cannot collide with any key
the host site reads.
