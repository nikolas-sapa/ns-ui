...
**Standing check — accent-tinted pointer highlights.** The single most repeated defect on this project: three shipped components (edge-yield, granule-churn, shear-billow) mixed `--ns-accent` into a pointer/beam/freshness highlight. `--ns-accent` is interaction-only chrome (buttons, focus rings) — a resting screenshot is not interaction. It hides well because `scripts/verify.ts` parks the pointer at the viewport origin, so the tint bakes a stationary coloured blob into the corner that reads as a rendering artifact, and no gate tests for hue. Rule: pointer highlights move in luminance only — add to L before the ramp, or brighten the colour directly, never mix in accent. Check: sample pixel R/G/B across the canvas in both themes and confirm they're equal within rounding everywhere except legitimate accent chrome (e.g. a CTA button). A grep for hex/`rgb()`/`hsl()` literals will not catch this — the hue comes from a token read at runtime and mixed in arithmetic.
...
## Measuring performance — the instruments lie, trust frame delivery

`gl.finish()`, `readPixels` fences (default framebuffer or FBO), and `EXT_disjoint_timer_query` all return a flat, non-moving 13-21ms on this stack's ANGLE/Metal backend regardless of draw count or viewport size — not "imprecise," actively wrong. This is a step past lesson (c) above, found the hard way after (c) was already on record: a component sat out of release for hours on a "20.9ms" reading that was pure artifact, real cost was 16.7ms. Two other agents separately hit the inverse fault, `gl.finish()` reporting 0.00ms. **Before trusting any timing number from these APIs, sanity-check the instrument by varying draw count or viewport and confirming the reading moves. If it doesn't move, the instrument is broken, not the component.**

Measure vsync-locked frame delivery instead: count frames and intervals over a sustained window, report p50/p90 interval and the count of frames over 20ms out of several hundred, both at rest and under sustained pointer motion. "16.7ms p50, 0 of 480 frames over 20ms" is a real verdict; a `readPixels`-fence number is not. Pair it with an A/B against a known-good sibling (weld-pool, dye-whorl) measured in the same session — matching a component already known to be smooth is stronger evidence than any absolute figure.

Two more conditions that invalidate a reading: machine load and headless rendering. Check `uptime` before measuring — components have been observed dropping their adaptive render ladder at load average 7-15 (concurrent agents) while holding full resolution at load ~5; report the load alongside the numbers. And verify where the adaptive ladder settles *unforced* at full DPR 2, not just that frame rate is acceptable — a component that only hits budget because the ladder quietly halved its resolution is a fail, the ladder is insurance, not a crutch. Separately, headless Chromium may be running on SwiftShader (software rasterizer); numbers from it are a floor, not a verdict — use headed Chromium via Playwright for any real-GPU claim.

Run the verifier as `BASE_URL=http://localhost:<port> npm run verify <slug>` — `npx tsx scripts/verify.ts` fails with `__name is not defined`.
