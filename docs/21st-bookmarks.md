# 21st.dev bookmarks → ns-ui gap map

Source: the 60 components bookmarked on 21st.dev (`21st.dev/community/bookmarks/components`,
pulled 2026-07-31). Extracted over CDP against the logged-in browser — one `Runtime.evaluate`
returning slugs, no snapshots, no screenshots.

Verdict column:

- **have** — ns-ui already answers the same need; nothing to build.
- **partial** — the mechanic exists here in a different register; a port would be a variation,
  not a new idea.
- **gap** — no ns-ui component solves this.
- **skip** — a page or template, not a component. Out of registry scope.

The registry's bar is "an interaction that does not already exist here", so **partial** is normally
a decline, not a backlog item.

## Shader / WebGL ambient surfaces (25 bookmarks)

The largest cluster by far, and the one ns-ui already answers hardest.

| Bookmark | Verdict | ns-ui |
|---|---|---|
| google-gemini-effect, spotlight-new | partial | `tour-spotlight`, `hero-isobar-contours` |
| wave-background, waves-shaders-homlu-ui, line-shader-homlu-ui | partial | `background-gradient-shader`, `background-ascii-plasma` |
| blue-meshy-background, gradient-bars-background, liquid-gradient | partial | `background-gradient-shader` |
| swirl, blue-light-swirl, verdant-swirl, shader-anima, magic-dust-shader | partial | `background-gradient-shader`, `hero-vortex-street` |
| glsl-hills, valley-of-the-mind, trail-grid | partial | `hero-ascii-terrain`, `grid-magnetic-lattice`, `breadcrumb-overflow-menu` |
| crystal-shader, liquid-crystal, icey-night-shards, wrap-shader | partial | `scroll-defrost`, `success-nucleation` |
| sphere, geometric-orb, torus-ish shaders | partial | `status-sphere-dots`, `ascii-torus-donut` |
| particles, seed, nebula | partial | `hero-particles-webgl`, `hero-vortex-street`, `hero-dipole-field` |
| knot-animation, symbols-effect, spooky-smoke-animation | gap | volumetric/knot geometry — nothing here does it |
| liquid-metal-hero, hero-liquid-metal, portfolio-hero-with-paper-shaders | **gap** | `border-chrome-ring` is metal on a *ring*, not a full-bleed hero |
| cobe-globe-satellites | partial | `ascii-globe-spin` (ASCII globe, no satellite arcs) |

## Glass, metal, buttons (7)

| Bookmark | Verdict | ns-ui |
|---|---|---|
| liquid-glass, apple-tahoe-liquid-glass-button, liquid-button | have | `button-glass`, `surface-glass` |
| shiny-button ×2, hero-button-expendable | partial | `button-glass`, `border-electric-arc`, `button-retry-backoff` |
| award-badge (Product Hunt badge) | skip | branded third-party badge |

## Text (7)

| Bookmark | Verdict | ns-ui |
|---|---|---|
| vertical-cut-reveal | partial | `reveal-cloth-unfurl`, `hero-letterpress-lockup` |
| morph-texts, text-dia | partial | `text-variable-weight`, `text-card-flick`, `hero-gravity-well` |
| animated-highlight-text | partial | `streaming-ink-dry`, `redaction-hold-reveal` |
| ai-text-loading | have | `progress-narrated`, `streaming-ink-dry` |
| text-word-carousel | **gap** | no rotating-word slot in the registry |
| pixel-perfect-hero, animated-hero-section | partial | `hero-ascii-wordmark`, `hero-recursive-type`, `scroll-story-strata` |

## Loading (5)

| Bookmark | Verdict | ns-ui |
|---|---|---|
| metamorphic-loader, progressive-flux-loader | have | `loader-iris`, `loader-spring-bars`, `loader-die-tumble`, `loader-pendulum-sync`, `loader-loom-weave` |
| preloader, 8bit-loading-screen | **gap** | `gel-wash` was built and removed — an invention, not a port of what was bookmarked |
| 8bit-not-found1 | have | `not-found-postmark`, `not-found-knockout` — both already here |

## Page furniture (5)

| Bookmark | Verdict | ns-ui |
|---|---|---|
| motion-footer, animated-footer | **gap** | `footing-course` was built and removed — a sticky reveal anyone can write, not what these two do |
| book-a-demo-2 | partial | `pricing-scale`, `wizard-dovetail` |
| preloader/404 | see above | |

## Data (3)

| Bookmark | Verdict | ns-ui |
|---|---|---|
| heatmaps | **gap → built** | `heatmap-calendar-tide` (this session) |
| funnel-chart | **gap** | no funnel/stage-drop chart |
| cobe-globe-satellites | partial | `ascii-globe-spin` |

## Templates, not components (6)

portfolio, istanblue, valley-of-the-mind, ricaelmenezes3/portfolio, nuova/shader-anima page,
easemize/pixel-perfect-hero page — **skip**. Whole-page compositions; the registry ships parts.

## Ranked build queue (the real gaps)

1. **Footer** — still open. `footing-course` was built and removed: it answered the *category*
   rather than either bookmarked component.
2. **Preloader / route curtain** — still open, same reason (`gel-wash`, removed).
3. ~~**Heatmap**~~ — built as `heatmap-calendar-tide` (loud, single-hue accent ramp).
4. **Liquid-metal full-bleed hero** — 3 bookmarks. `border-chrome-ring` proves the shader; the gap is
   the hero-scale surface with type sitting in it. Biggest remaining, and a WebGL job.
5. **Rotating-word slot** — 1 bookmark, but the one text mechanic genuinely absent.
6. **Funnel chart** — narrower need than the heatmap, same bucket.
7. ~~**404 state**~~ — already covered twice (`not-found-postmark`, `not-found-knockout`); the original map was
   wrong about this one.

Method note, learned the hard way: this map was built from slugs alone. Classifying by name is fine
for deciding *whether* something is a gap; it is not enough to build against. Anything built from a
bookmark must start by reading that component's real source — its timings, easings and mechanics —
not by inventing a house-style answer to its category. Two components were built that way and both
were removed.

Everything else in the 60 is already answered here or is a template.
