...
export function CommandPalette({
  ...
  onClose: () => void;
  /**
   * Every component, name and title, handed down from `SiteShell`'s fetched
   * nav tree — `null` until that fetch lands.
   *
   * This module used to `import registry from
   * "@/lib/registry-lite.generated.json"` for exactly these two fields. A
   * JSON import is inlined verbatim by the bundler, so those 534 entries
   * (89.2 KB minified, worth 21.0 KB gz of `433s_kwy47172.js` — measured by
   * stripping it from the built chunk) sat in a chunk loaded by 562 of the 567
   * prerendered documents, parsed once per document and twelve more times per
   * gallery view inside the card iframes, to serve a palette most visits never
   * open. Cutting only the sidebar's import edge would have left every byte of
   * it in place; both importers had to go. The shell already fetches the same
   * list for the sidebar, so taking it as a prop costs nothing.
   */
  items: NavItem[] | null;
  ...
  const results = useMemo<Result[]>(() => {
    ...
    for (const item of items ?? []) {
      // One slot short of the cap: the escape hatch below is appended after
      // this and must not be crowded out by component hits.
    ...
    // title only (the nav tree carries no other field), while the catalog's
